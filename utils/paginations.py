...
class EndlessPagination(BasePagination):
    ...
    def paginate_ordered_list(self, reverse_ordered_list, request):
        ...
        if 'created_at__lt' in request.query_params:
            # integrate iso and int format
            try:
                created_at__lt = parser.isoparse(request.query_params['created_at__lt'])
            except ValueError:
                created_at__lt = int(request.query_params['created_at__lt'])
            for index, obj in enumerate(reverse_ordered_list['created_at__lt']):
    ...
    def paginate_queryset(self, queryset, request, view=None):
        ...
        queryset = queryset.order_by('-id')[:self.page_size + 1] # get one data more
        self.has_next_page = len(queryset) > self.page_size
        return queryset[:self.page_size]

    def paginate_hbase(self, hb_model, row_key_prefix, request):
        if 'created_at__gt' in request.query_params:
            create_at__gt = request.query_params['create_at__gt']
            start = (*row_key_prefix, create_at__gt)
            stop = (*row_key_prefix, MAX_TIMESTAMP)
            objects = hb_model.filter(start=start, stop=stop)
            if len(objects) and objects[0].created_at == int(create_at__gt):
                objects = objects[:0:-1]
            else:
                objects = objects[::-1]
            self.has_next_page = False
        ...
        if 'created_at__lt' in request.query_params:
            ...
            if len(objects) and objects[0].created == int(created_at__lt):
                objects = objects[1:]
            if len(objects) > self.page_size:
                ...
                self.has_next_page = False
            return objects
        # no params, used the latest page
