...
class HBaseModel:
    ...
    @classmethod
    def get_table(cls):
        conn = HBaseClient.get_connection() # singleton
    ...
    def deserialize_row_key(cls, row_key):
        ...
        for key in cls.Meta.row_key:
            ...
            if index == -1:
                break
            data[key] = cls.deserialize_row_key(key, row_key[:index])
    ...
    @classmethod
    def deserializer_field(cls, key, value):
    ...
    @classmethod
    def serializer_row_data(cls, data):
        row_data = {}
        field_hash = cls.get_field_hash()
        for key, field in field_hash.items():
    ...
    def save(self, batch=None):
        row_data = self.serializer_row_data(self.__dict__)
        if len(row_data) == 0:
    ...
    def create_table(cls):
        ...
        column_families = {
            ...
            for key, field in cls.get_field_hash().items() if field.column_family is not None
        }
    ...
    def serialize_row_key_from_tuple(cls, row_key_tuple):
        if row_key_tuple is None:
            return None
        data = {
            key: value for key, value in zip(cls.Meta.row_key, row_key_tuple)
        }
